...
public class RenderRightAwaySignal<T extends RightAwaySignal.TileEntityDispatchSignal> extends BlockEntityRenderer<T> implements IGui, IDrawing {
    ...
    @Override
    public void render(@Nonnull RightAwaySignal.TileEntityDispatchSignal entity, float tickDelta, @Nonnull GraphicsHolder graphicsHolder, int light, int overlay) {
        ...
        if (!detectedColors.isEmpty()) {
            ...
            if (!aspects.right().intStream().anyMatch((a) -> {
                ...
                final MutableText roundelText = TextHelper.setStyle(TextHelper.literal(IGui.textOrUntitled("RA")), style);
