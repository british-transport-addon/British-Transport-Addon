...
public class LondonBusStopSignRender<T extends LondonBusStopSign.TileEntityLondonBusSign> extends BlockEntityRenderer<T> implements IGui, IDrawing {
    ...
    public LondonBusStopSignRender(Argument dispatcher, float maxWidth, float maxScale, float xOffset, float yOffset, float zOffset, float xTilt, int textColor, Identifier font) {
        super(dispatcher);
    ...
    public void render(@Nonnull T entity, float tickDelta, @Nonnull GraphicsHolder graphicsHolder, int light, int overlay) {
        ...
        MainRenderer.scheduleRender(QueuedRenderLayer.TEXT, (graphicsHolderNew, offset) -> {
            ...
            // facing traffic side.
            renderWhite(graphicsHolderNew, streetText, textWidth + 3, light);
            graphicsHolderNew.translate(0, .1, 0);
            renderWhite(graphicsHolderNew, streetTextTwo, textWidthTwo + 3, light);
            ...
            render(graphicsHolderNew, servicesTextOne, servicesTextOneWidth - 7, light);
            graphicsHolderNew.rotateYDegrees(180);
            render(graphicsHolderNew, servicesTextOne, servicesTextOneWidth - 7, light);
    ...
    private void renderWhite(GraphicsHolder graphicsHolder, MutableText roundelText, int textWidth, int light) {
        graphicsHolder.push();
        graphicsHolder.rotateXDegrees(xTilt);
        graphicsHolder.translate(-xOffset, -yOffset, -zOffset - SMALL_OFFSET * 2);
        ...
        graphicsHolder.scale(scale, scale, scale);
        graphicsHolder.translate(0, -3.5, 0);
        graphicsHolder.drawText(roundelText, -textWidth / 2, 0, 0xFFFFFFFF, false, light);
