...
public class PlatformNumberSignRender<T extends PlatformNumberSign.PlatfornNumberSignEntity> extends BlockEntityRenderer<T> implements IGui, IDrawing {
    ...
    public PlatformNumberSignRender(Argument dispatcher, float maxWidth, float maxScale, float xOffset, float yOffset, float zOffset, float xTilt, int textColor, Identifier font) {
        super(dispatcher);
    ...
    public void render(@Nonnull T entity, float tickDelta, @Nonnull GraphicsHolder graphicsHolder, int light, int overlay) {
        ...
        MainRenderer.scheduleRender(QueuedRenderLayer.TEXT, (graphicsHolderNew, offset) -> {
            ...
            render(graphicsHolderNew, entity.getPlatformNumber(), GraphicsHolder.getTextWidth(entity.getPlatformNumber()), light);

            graphicsHolderNew.rotateYDegrees(180);
            graphicsHolderNew.translate(-.45, 0, 0);
            render(graphicsHolderNew, entity.getPlatformNumber(), GraphicsHolder.getTextWidth(entity.getPlatformNumber()), light);
