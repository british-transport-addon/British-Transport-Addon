...
import org.eu.awesomekalin.jta.mod.blocks.directional.street.LondonStreetSign2x2;
...
public class LondonStreetSign2x2Render<T extends LondonStreetSign2x2.LondonStreetSign2x2BlockEntity> extends BlockEntityRenderer<T> implements IGui, IDrawing {
    ...
    public LondonStreetSign2x2Render(Argument dispatcher, float maxWidth, float maxScale, float xOffset, float yOffset, float zOffset, float xTilt, int textColor, Identifier font) {
        super(dispatcher);
    ...
    public void render(@Nonnull T entity, float tickDelta, @Nonnull GraphicsHolder graphicsHolder, int light, int overlay) {
        ...
        MainRenderer.scheduleRender(QueuedRenderLayer.TEXT, (graphicsHolderNew, offset) -> {
            ...
            graphicsHolderNew.translate(.7, 0, 0);
            renderRed(graphicsHolderNew, entity.getLine2(), GraphicsHolder.getTextWidth(entity.getLine2()), light);
            graphicsHolderNew.translate(-.45, .25, 0);
            renderRed(graphicsHolderNew, entity.getLine3(), GraphicsHolder.getTextWidth(entity.getLine3()) + 8, light);
    ...
    private void renderRed(GraphicsHolder graphicsHolder, MutableText roundelText, int textWidth, int light) {
        graphicsHolder.push();
        graphicsHolder.rotateXDegrees(xTilt);
        graphicsHolder.translate(-xOffset, -yOffset, -zOffset - SMALL_OFFSET * 2);

        final float scale = Math.min(((maxWidth / 4) * 3) / textWidth, maxScale);
        graphicsHolder.scale(scale, scale, scale);
        graphicsHolder.translate(0, -3.5, 0);
        graphicsHolder.drawText(roundelText, -textWidth / 2, 0, 0xFFAA0000, false, light);
